...
# Modified for [RoomRecon] (https://github.com/shenckens/RoomRecon) by Stijn Henckens.
...
class ToTensor(object):
    ...
    def __call__(self, data):
        data['imgs'] = torch.Tensor(
            np.stack(data['imgs']).transpose([0, 3, 1, 2]))
        ...
        if 'tsdf_list_full' in data.keys():
            for i in range(len(data['tsdf_list_full'])):
                if not torch.is_tensor(data['tsdf_list_full'][i]):
                    data['tsdf_list_full'][i] = torch.Tensor(
                        data['tsdf_list_full'][i])
...
class IntrinsicsPoseToProjection(object):
    ...
    def rotate_view_to_align_xyplane(self, Tr_camera_to_world):
        ...
        z_c = np.dot(np.linalg.inv(Tr_camera_to_world),
                     np.array([0, 0, 1, 0]))[: 3]
    ...
    def __call__(self, data):
        ...
        data['world_to_aligned_camera'] = torch.from_numpy(
            rotation_matrix4x4).float() @ middle_pose.inverse()
        data['middle_pose'] = middle_pose
        data['camera_to_aligned_camera'] = torch.from_numpy(
            rotation_matrix4x4).float()
...
class GeneratePlaneGT(object):
    ...
    def __init__(self, normal_anchor_path):
        self.normal_anchors = torch.from_numpy(
            np.load(normal_anchor_path)).float()
        ...
        if 'planes' in data.keys():
            if len(data['planes']) != 0:
                ...
                planes_gt = (data['middle_pose'].transpose(0,
                    ...
                    0, 1)).transpose(0, 1)
                # reduce 2*pi to pi
                inverse_id = torch.nonzero(
                    planes_gt[:, 2] > 0, as_tuple=False).squeeze(1)
                ...
                distances = torch.norm(normal_gt.unsqueeze(
                    1) - self.normal_anchors.unsqueeze(0), dim=-1)
...
class RandomTransformSpace(object):
    ...
    def transform(self, data, transform=None, old_origin=None,
        ...
        Returns:
            Items with new TSDF, plane parameters and occupancy in the transformed coordinates
        """
        ...
        for i in range(data['imgs'].shape[0]):
            ...
            view_frust_pts = get_view_frustum(
                self.max_depth, size, cam_intr, cam_pose)
            bnds[:, 0] = torch.min(
                bnds[:, 0], torch.min(view_frust_pts, dim=1)[0])
            bnds[:, 1] = torch.max(
                bnds[:, 1], torch.max(view_frust_pts, dim=1)[0])

        # -------adjust volume bounds-------
        num_layers = 3
        center = (torch.tensor(((bnds[0, 1] + bnds[0, 0]) / 2,
                                (bnds[1, 1] + bnds[1, 0]) / 2,
                                -0.2)) - data['vol_origin']) / self.voxel_size
        center[:2] = torch.round(
            center[:2] / 2 ** num_layers) * 2 ** num_layers
